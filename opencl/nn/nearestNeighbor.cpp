...
int main(int argc, char* argv[])
{
    ...
    // args
    char filename[100];
    int resultsCount = 10, quiet = 0, timing = 0, platform = -1,
        device = -1;
    float lat = 0.0, lng = 0.0;

    // parse command line
    if (parseCommandline(argc, argv, filename, &resultsCount, &lat,
            &lng, &quiet, &timing, &platform, &device)) {
        printUsage();
        return 0;
    }
    ...
    recordDistances = OpenClFindNearestNeighbors(
        context, numRecords, locations, lat, lng, timing);
...
float* OpenClFindNearestNeighbors(cl_context context, int numRecords,
    std::vector<LatLong>& locations, float lat, float lng, int timing)
{
    ...
    cl_NN_program
        = cl_compileProgram((char*)"nearestNeighbor_kernel.cl", NULL);

    NN_kernel
        = clCreateKernel(cl_NN_program, "NearestNeighbor", &status);
    status = cl_errChk(
        status, (char*)"Error Creating Nearest Neighbor kernel", true);
    if (status)
        exit(1);
    ...
    error = clEnqueueWriteBuffer(command_queue, d_locations,
        ...
        0, // offset
        sizeof(LatLong) * numRecords, &locations[0], 0, NULL,
        &writeEvent);

    // 3. send arguments to device
    cl_int argchk;
    argchk = clSetKernelArg(
        NN_kernel, 0, sizeof(cl_mem), (void*)&d_locations);
    argchk |= clSetKernelArg(
        NN_kernel, 1, sizeof(cl_mem), (void*)&d_distances);
    argchk |= clSetKernelArg(
        NN_kernel, 2, sizeof(int), (void*)&numRecords);
    argchk |= clSetKernelArg(NN_kernel, 3, sizeof(float), (void*)&lat);
    argchk |= clSetKernelArg(NN_kernel, 4, sizeof(float), (void*)&lng);

    cl_errChk(
        argchk, "ERROR in Setting Nearest Neighbor kernel args", true);
...
#define NUM_ITERATIONS 10000
    ...
    if (timing) {
        ...
        printf("%d        \t", numRecords);
        // Write Buffer
        error = clGetEventProfilingInfo(writeEvent,
            CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &eventStart,
            NULL);
        cl_errChk(
            error, "ERROR in Event Profiling (Write Start)", true);
        error = clGetEventProfilingInfo(writeEvent,
            CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &eventEnd,
            NULL);
        ...
        // Kernel
        for (int tc = 0; tc < NUM_ITERATIONS; tc++) {
            error = clGetEventProfilingInfo(kernelEvent[tc],
                CL_PROFILING_COMMAND_START, sizeof(cl_ulong),
                &eventStart, NULL);
            cl_errChk(
                error, "ERROR in Event Profiling (Kernel Start)", true);
            error = clGetEventProfilingInfo(kernelEvent[tc],
                CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &eventEnd,
                NULL);
            cl_errChk(
                error, "ERROR in Event Profiling (Kernel End)", true);
        ...
        // Read Buffer
        error = clGetEventProfilingInfo(readEvent,
            CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &eventStart,
            NULL);
    ...
    float* lat, float* lng, int* q, int* t, int* p, int* d)
{
    ...
    if ((*d >= 0 && *p < 0) || (*p >= 0 && *d < 0)) // both p and d must
                                                    // be specified if
                                                    // either are
                                                    // specified
        return 1;
...
void printUsage()
{
    ...
    printf("\n");
    printf("nearestNeighbor [filename] -r [int] -lat [float] -lng "
           "[float] [-hqt] [-p [int] -d [int]]\n");
    printf("\n");
    printf("example:\n");
    ...
    printf("-lat [float] the latitude for nearest neighbors (default: "
           "0)\n");
    printf("-lng [float] the longitude for nearest neighbors (default: "
           "0)\n");
    printf("\n");
    ...
    printf("\n");
    printf("-p [int]     Choose the platform (must choose both "
           "platform and device)\n");
    printf("-d [int]     Choose the device (must choose both platform "
           "and device)\n");
    printf("\n");
    printf("\n");
    printf(
        "Notes: 1. The filename is required as the first parameter.\n");
    printf("       2. If you declare either the device or the "
           "platform,\n");
